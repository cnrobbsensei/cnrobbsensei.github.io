...
(function(){
  ...
  window.cnSession = {
    ...
    async findActivePointsEntry(name){
      ...
      const matches = all.filter(e => e.date === today && e.name && e.name.trim().toLowerCase() === trimmed.toLowerCase());
      ...
      return {ok:true, matchedName: matches[0].name.trim(), enteredAt: mostRecent, expiresAt: mostRecent + window.SESSION_MS};
    },
